...
var fm = (function($) {
  ...
  function getFmOptions(event, position) {
    ...
    if ($(event.target).closest(".feedback_trigger").length === 1) {
      ...
      if ($(event.target).closest(".feedback_content").length === 1) {
    ...
    if (className.indexOf('left-top') !== -1) {
      ...
      if (className.indexOf('left-bottom') !== -1) {
        selector = 'left-bottom';
      } else {
        if (className.indexOf('right-top') !== -1) {
          selector = 'right-top';
        } else {
          if (className.indexOf('right-bottom') !== -1) {
  ...
  function validateFeedbackForm(event, position) {
    ...
    if (
      (fm_options.name_required === true && $fm_content.find(".feedback_name").val() === "") ||
      ((fm_options.email_required === true && $fm_content.find(".feedback_email")
        .val() === "") || (fm_options.email_required === true && emailValid($fm_content.find(".feedback_email")
        .val()) === false)) ||
      (fm_options.message_required === true && $fm_content.find(".feedback_message").val() === "") ||
      (fm_options.radio_button_list_required === true && $fm_content.find("input[name=feedback_radio]:checked")
        .val() === undefined)) {
      return false;
    }
    return true;
  ...
  function appendFeedbackToBody(fm_options) {
    var form_html = "",
      ...
      county_pattern = fm_options.county_pattern === "" ? "" : "pattern=\"" + fm_options.county_pattern + "\"",
      document_pattern = fm_options.document_pattern === "" ? "" : "pattern=\"" + fm_options.document_pattern + "\"",
      subject_pattern = fm_options.subject_pattern === "" ? "" : "pattern=\"" + fm_options.subject_pattern + "\"",
      
      name_required = fm_options.name_required ? "required" : "",
      email_required = fm_options.email_required ? "required" : "",
      county_required = fm_options.county_required ? "required" : "",
      document_required = fm_options.document_required ? "required" : "",
      subject_required = fm_options.subject_required ? "required" : "",
      topic_required = fm_options.topic_required ? "required" : "",
      message_required = fm_options.message_required ? "required" : "",
      accept_required = fm_options.accept_required ? "required" : "",
    ...
    if (fm_options.show_email === true) {
      email_html = '<li> <input type="email" class="feedback_email" ' + email_required + ' placeholder="' + fm_options.email_placeholder + '"></input> </li>';
      email_feedback_content_class = " email_present";
    }
    
    if (fm_options.show_form === true) {
      form_html = '<div class="feedback_modal_content"><form class="feedback_me_form">'
        + '<ul>'
        
        + '<li>	<input type="text" class="feedback_name" ' + name_required + ' placeholder="' + fm_options.name_placeholder + '" ' + name_pattern + '></input> </li>'
        
        + email_html
        
        + '<li>' +
        '<select class="feedback_county" ' + county_required + '><option value="">- Select County -</option>' +
        ...
        + '<li>' +
        '<select class="feedback_document" ' + document_required + '><option value="">- Select Document -</option>' +
        '<option>Draft Plan Document</option>' +
        '<option>Draft EIR</option>' +
        '<option>Draft Supplemental Report(s)</option>' +
        '</select>' +
        '</li>'
        
        + '<li>' +
        '<select class="feedback_topic" ' + topic_required + '><option value="">- Select Topic -</option>' +
        '<option>Economy & Jobs</option>' +
        '<option>Environmental Quality</option>' +
        '<option>Equity</option>' +
        '<option>Freight/Goods Movement</option>' +
        '<option>Funding (transportation projects or revenues)</option>' +
        ...
        + '<li>	<input type="text" class="feedback_subject" ' + subject_required + ' placeholder="' + fm_options.subject_placeholder + '" ' + subject_pattern + '></input> </li>'
        
        + '<li>	<textarea rows="5" class="feedback_message" ' + message_required + ' placeholder="' + fm_options.message_placeholder + '"></textarea> </li>'
        
        + radio_button_list_html
        
        + '<li>	<input type="checkbox" class="feedback_accept" value="1" checked="checked" id="accept_checkbox" ' + accept_required + '> <label class="accept_checkbox" for="accept_checkbox">Sign me up for Plan Bay Area 2040 updates by email</label> </li>'
    ...
    $('body')
      .append('<div onclick="fm.stopPropagation(event);fm.triggerAction(event,\'' + fm_options.position + '\');" class="feedback_trigger feedback_trigger_closed ' + fm_options.position + jQueryUIClasses1 + fm_class + jquery_class + bootstrap_class + bootstrap_hero_unit + '">'
        + '<span class="feedback_trigger_text">' + fm_options.trigger_label
        + '</span></div>');
    
    $('body')
      .append('<div class="feedback_content feedback_content_closed ' + fm_options.position + email_feedback_content_class + radio_button_list_class + jQueryUIClasses2 + fm_class + jquery_class + bootstrap_class + bootstrap_hero_unit + '">'
        + '<div class="feedback_title ' + jQueryUIClasses1 + jQueryUIClasses3 + '">'
        + '<h3 class="feedback_header ' + jQueryUIClasses4 + '">' + fm_options.title_label + '</h3>'
        + '</div>'
        + form_html
        + iframe_html
  ...
  function slideBack(fm_options, $fm_trigger, $fm_content, delay) {
    
    if ( delay ) {
      setTimeout(function(){ slideBack(fm_options, $fm_trigger, $fm_content); }, 5000);
      return;
    }
  ...
  function sendFeedback(event, position) {
    ...
    if ( $fm_content.find(".feedback_accept").is(':checked') ) {
      var accept = 1;
    }
    else {
      var accept = 0;
    ...
    dataArray = {
      ...
      county          : $fm_content.find(".feedback_county").val(),
      document          : $fm_content.find(".feedback_document").val(),
      topic           : $fm_content.find(".feedback_topic").val(),
      subject         : $fm_content.find(".feedback_subject").val(),
...
//        if (fm_options.delayed_close === true) {
          if (fm_options.delayed_options.custom_html_success === undefined) {
            ...
            slideBack(fm_options, $fm_trigger, $fm_content, 3000);
            
            $(".feedback_modal_content").html( fm_options.delayed_options.send_success );
      ...
      error     : function(ob, errStr) {
        ...
        if (fm_options.delayed_close === true) {
          if (fm_options.delayed_options.custom_html_fail === undefined) {
            ...
            $fm_content.find('.feedback_submit').text(fm_options.submit_label);
            $("body")
              .append('<div ' + st + ' class="feedback-delayed-dlg fail" onclick="fm.stopPropagation(event);"><span class="feedback-dlg-close" onclick="fm.closeFeedbackDelayedDlg();">X</span><span class="feedback-fail-message">' +
            ...
            $fm_content.find('.feedback_submit').text(fm_options.submit_label);
            $("body")
              .append('<div class="feedback-delayed-custom-dlg fail" onclick="fm.stopPropagation(event);">' + fm_options.delayed_options.custom_html_fail + '</div>');
            setTimeout(function() {
...
//			name_label : false,
        ...
        delayed_options           : {
          ...
          send_success              : "<div style='padding: 20px;'>Thank you for your comment. We will share all the comments we receive on the draft plan with decision-makers prior to any final action on Draft Plan Bay Area 2040. A summary of what we heard will be presented to MTC and ABAG in June. Visit PlanBayArea.org for more information. Please submit any other written comments on or before the June 1, 2017 public comment deadline.</div>",
          fail_color                : undefined,
          success_color             : undefined,
